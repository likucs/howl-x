...
if Var.LOAD_MYBOT == "True":
    USER_BOT_NO_WARN = (
        "**𝙷𝙴𝚈 𝚃𝙷𝙸𝚂 𝙸𝚂 𝙷𝙾𝚆𝙻 𝙿𝙼 𝚂𝙴𝙲𝚄𝚁𝙸𝚃𝚈 !!! 𝙷𝙴𝚁𝙴 𝚃𝙾 𝙿𝚁𝙾𝚃𝙴𝙲𝚃 [{}](tg://user?id={})**\n\n"
        "{}\n\n"
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("`ңøωȴ"):
            rev_text = query[::-1]
            but = [[custom.Button.inline("💬 Oᴘᴇɴ ʜᴇʟᴘ ᴍᴇɴᴜ »»", data="menu")]]
            but += [[custom.Button.inline("💡 Pɪɴɢ »»", data="pkng")]]
            but += [[Button.url("🔰 Sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ »»", "t.me/discuss_group_cs"), Button.url("🔰 Uᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ", "t.me/mod_apk_premium_cs")]]
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = (
                "Please get your own Userbot from contact @discuss_group_cs , and don't use mine!"
            )
